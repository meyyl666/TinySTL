...
#define DEQUE_MAP_INIT_SIZE 8
	...
	struct deque_iterator :public iterator<random_access_iterator_tag, T>
	{
		...
		value_pointer last;    //指向 所在缓冲区的尾部
		...
		deque_iterator(iterator&& rhs) noexcept   //移动操作不应抛出任何异常
			:cur(rhs.cur), first(rhs.first), last(rhs.last), node(rhs.node)
		{
			rhs.cur = nullptr;
			rhs.first = nullptr;
	...
	//模板参数代表数据类型
	 ...
	 public:
		 ...
		 explicit deque(size_type n)    //explicit 只能修饰含有一个参数的构造函数，表示该函数为显式构造
		 {
		 ...
		 ~deque()
		 {
			 if (map_ != nullptr)
			 {
				 ...
				 *begin_.node = nullptr;
				 map_iterator::deallocate(map_, map_size_);
				 map_ = nullptr;
			 }
		 }
	 ...
	 public:
		 ...
		 reverse_iterator rbrgin() noexcept
		 {
		 ...
		 void push_fornt(value_type&& value) { emplace_front(mystl::move(value)); }
		 ...
		 private:
			 ...
			 template <class ITer>
			 void copy_init(ITer, ITer, input_iterator_tag);
	 ...
	 deque<T>& deque<T>::operator=(const deque& rhs)
	 {
		 if (this != &rhs)
		 {
			 ...
			 if (len >= rhs.size())
			 {
				 erase(mystl::copy(rhs.begin_, rhs.end_, end_));
			 }
			 else   //原数据空间不够用
			 {
				 iterator mid = rhs.begin() + static_cast<difference_type>(len);
				 mystl::copy(rhs.begin_, mid, begin_);
				 insert(end_, mid, rhs.end_);   //在末尾追加
			 }
	 ...
	 void deque<T>::emplace_front(Args&& ...args)
	 {
		 ...
		 else
		 {
			 ...
			 try {
				 --begin_;
				 data_allocator::contruct(begin_.cur, mystl::forward<Args>(args)...);
			 }
			 catch (...)
			 {
				 ++begin_;
	 ...
	 void deque<T>::map_init(size_type nElem)
	 {
		 ...
		 //让nstart he nfinish 都指向map_最中央区域，方便向头尾扩充
		 ...
		 end_cur = end_.first + (nElem % buffer_size);
	 ...
	 void deque<T>::copy_init(IIter first, IIter last, input_iterator_tag)
	 {
		 ...
		 {
			 auto next = first;
			 mystl::adcance(next, buffer_size);
	 ...
	 {
		 const size_type elems_before = position - begin_;   //插入点之前的个数
