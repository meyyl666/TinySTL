...
namespace mystl {
	...
	public :
		...
		queue(Container&&  c) noexcept(std::is_nothrow_constructible<Container>::value)
		 ...
		 queue(queue&& rhs) noexcept (std::is_nothrow_constructible<Container>::value)
		 ...
		 queue& operator=(deque&& rhs) noexcept(std::is_nothrow_move_constructible<Container>::value)
		 {
			 c_ = mystl::move(rhs.c_);
			 return *this;
		 }
		 ...
		 const_reference back() { return c_back(); }

		 //容量相关操作
		 bool empty() const noexcept { return c_.empty(); }
		 size_type size() const noexcept { return c_size(); }
	...
	private:
		container_type    c_;   //用底层容器实现 priority_queue
	...
	public:
		...
		priority_queue(priority_queue&& rhs)
			:c_(mystl::move(rhs.c_)), comp_(mystl::move(rhs.comp_))
		{
			mystl::make_heap(c_.begin(), c_end(), comp_);
		}
	...
	public:
		...
		{
			c_.emplace_back(mystl::forward<Args>(args)...);
			mystl::push_heap(c_.begin(), c_end(), comp_);
		...
		void push(value_type&& value)
		{
			...
			mystl::push_heap(c_.begin(), c_end(), copm_);

		}

		void pop()
		{
			mystl::pop_heap(c_.begin(), c_end(), comp_);
			c_.pop_back();
		}
