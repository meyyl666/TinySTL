...
namespace mystl {
	//模板类 set，键值不允许重复
	//参数一代表键值类型，参数二代表键值的比较方式，缺省使用mystl::less
	...
	private:
		//mystl::rb_tree 作为底层机制
		typedef mystl::rb_tree<value_type, value_compare>  base_type;
		base_type tree_;
	public:
		//使用rb_tree定义型别
		typedef typename base_type::node_type             node_type;
		typedef typename base_type::const_pointer         painter;
		typedef typename base_type::const_pointer         const_painter;
		...
		typedef typename base_t-ype::size_type             size_type;
	...
	public:
		...
		size_type erase(const key_value& key) { return tree_.erase_unique(key); }
		void erase(iterator first, iterator last) { return tree_.erase(first, last); }
		...
		iterator upper_bound(const key_type& key) { return tree_.upper_bound(); }
		const_iterator upper_bound(const key_value& key) { return tree_.upper_bound(); }
	...
	public:
		...
		{
			tree_insert_multi(ilist.begin(), ilist.end());
		}
		...
		iterator insert(iterator hint, value_type& value)
		{
			return tree_.insert_multi_use_hint(value);
		}
		iterator insert(iterator hint, value_type&& value)
		{
			return tree_.insert_multi_use_hint(hint, mystl::move(value));
		}
		...
		size_type count(const key_type& key) { return tree_.count_mulit(key); }
		...
		pair<iterator,iterator>
			equal_range(const keuy_type& key)
		{
		...
		pair<iterator,iterator>
			equal_range(const key_type& key)
		{
			return tree_.equal_range_mulit(key);
		}
