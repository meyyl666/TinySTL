...
namespace mystl {
	...
	//模板类 srack
	//参数一代表数据类型  参数二代表底层容器类型，缺省使用 mystl::deque作为底层容器
	...
	public:
		...
		static_assert(std::is_same<T, value_type>::value, "the calue_type of Container should be same with T");
	...
	public :
		...
		stack(Container& c) :c_(c)
		{

		}
		stack(Container&& c):c_(mystl:move(c)){}

		stack(Container&& c)noexcept(std::is_nothrow_constructible<Container>::value):c_(stl::move(c)){}
		...
		public:
			friend bool operator==(const stack& lhs, const stack& rhs) { return lhs.c_ == lhs.c_; }
